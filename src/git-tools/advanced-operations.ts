...
import type {GitOperationResponse, GitProviderFactory} from "../providers/git-provider";

export const configureAdvancedOperations = (getGit: GitProviderFactory) => ({
    ...
    gitStashPop: async (args: { index?: number; repoPath?: string }): Promise<GitOperationResponse> => {
        try {
            const git = await getGit({repoPath: args.repoPath});
    ...
    gitCherryPick: async (args: { commit: string; repoPath?: string }): Promise<GitOperationResponse> => {
        try {
            const git = await getGit({repoPath: args.repoPath});
