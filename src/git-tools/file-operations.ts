...
import {GitProviderFactory, GitOperationResponse, GitDiffFileResult, GitDiffSummary} from '../providers/git-provider';
...
export const configureFileOperations = (getGit: GitProviderFactory) => ({
  ...
  gitReset: async (args: { files?: string[]; mode?: 'soft' | 'mixed' | 'hard'; commit?: string; repoPath?: string }): Promise<GitOperationResponse> => {
    try {
      const git = await getGit({ repoPath: args.repoPath });
  ...
  gitRestore: async (args: { files: string[]; staged?: boolean; repoPath?: string }): Promise<GitOperationResponse> => {
    try {
      const git = await getGit({ repoPath: args.repoPath });
